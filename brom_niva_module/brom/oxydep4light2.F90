...
!     Variable identifiers
      ...
      type (type_dependency_id)            :: id_par,id_temp,id_salt,id_depth
      type (type_dependency_id)            :: id_julianday
...
!     Model parameters
      !----Phy -----------!
       real(rk) :: Max_uptake, bm, cm, Knut, r_phy_nut, r_phy_pom, r_phy_dom, r_phy_pom_anox,ir_min, O2_add_mor_phy
       real(rk) :: q10, t_upt_min, t_upt_max, lat_light, Io, Iopt, k_Erlov
      !----Het -----------!
...
!BOC
  ...
  ! PHY
   ...
   call self%get_parameter(self%lat_light, 'lat_light', 'degrees north',  'Latitude of modelled site ',                      default=25.0_rk)
   call self%get_parameter(self%Io,        'Io',    'Watts/m**2/h',  'Theoretical maximum 24-hr average surface downwelling shortwave irradiance in air [W/m2]', default=80.0_rk)
   call self%get_parameter(self%Iopt,      'Iopt',    'Watts/m**2/h', 'Optimal irradiance',                                 default=25.0_rk)
   call self%get_parameter(self%k_Erlov,    'k_Erlov',   '1/m',       'Background PAR attenuation',                          default=0.01_rk)
  ...
  ! Upper boundary for oxygen flux calculations
   ...
   call self%register_state_variable(self%id_phy,'Phy','mmol/m**3','Phy: Phytoplankton',  0.1_rk, minimum=0.0_rk, vertical_movement=self%Wphy)
   ...
   call self%register_state_variable(self%id_het,'Het','mmol/m**3','Het: Heterotrophs',  0.1_rk, minimum=0.0_rk, vertical_movement=self%Whet)
...
!   _GET_(self%id_par,Iz)             ! local photosynthetically active radiation
   ...
   _GET_(self%id_depth,depth)         ! temperature
   _GET_GLOBAL_(self%id_julianday,jday) ! julian day
!   _GET_(self%id_julianday,julianday) ! julian day

!--------------------------------------------------------------
! Phy
!--------------------------------------------------------------
   ! Growth of Phy and uptake of NUT
         decline = 23.5_rk*sin(2.0_rk*3.141519_rk*(real(jday,rk)-81.0_rk)/365.0_rk) !Solar declination in degrees
         Iot = max(0.0_rk, self%Io*cos((self%lat_light-decline)*3.14159_rk/180.0_rk)) ! Surface shortwave downwelling irradiance [W m^-2, 24-hr average]
        !This is the approximation used in Yakushev and Sorensen (2013) for OXYDEP
         Iz=Iot*exp(-self%k_Erlov*depth)
